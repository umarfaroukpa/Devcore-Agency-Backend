...
exports.getSystemInfo = exports.validateSettings = exports.importSettings = exports.exportSettings = exports.resetSettings = exports.getSettingSection = exports.testEmailSettings = exports.updateSettings = exports.getSettings = void 0;
...
exports.updateSettings = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (settingsData.general && (!settingsData.general.siteName || !settingsData.general.siteUrl)) {
        throw new ErrorHandler_1.AppError('Site name and URL are required', 400);
    }
    ...
    if (updatedSettings.notifications.emailNotifications) {
        if (!updatedSettings.email.smtpHost || !updatedSettings.email.smtpPort || !updatedSettings.email.smtpUsername) {
            throw new ErrorHandler_1.AppError('SMTP configuration is required when email notifications are enabled', 400);
        }
    }
    // Save to file
    ...
    if (settingsData.general) {
        if (settingsData.general.siteName)
            process.env.SITE_NAME = settingsData.general.siteName;
        if (settingsData.general.siteUrl)
            process.env.SITE_URL = settingsData.general.siteUrl;
        if (settingsData.general.adminEmail)
            process.env.ADMIN_EMAIL = settingsData.general.adminEmail;
    }
    res.status(200).json({
        success: true,
...
// Test email settings
exports.testEmailSettings = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const { email } = req.body;
    if (!email) {
        throw new ErrorHandler_1.AppError('Email address is required for testing', 400);
    }
...
exports.getSettingSection = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const { section } = req.params;
    const validSections = ['general', 'security', 'notifications', 'email'];
    if (!validSections.includes(section)) {
        throw new ErrorHandler_1.AppError('Invalid settings section', 400);
    }
    const settings = await loadSettings();
    res.status(200).json({
        success: true,
...
exports.exportSettings = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const settings = await loadSettings();
    res.status(200).json({
        success: true,
        data: {
...
exports.importSettings = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const { settings } = req.body;
    if (!settings) {
        throw new ErrorHandler_1.AppError('Settings data is required for import', 400);
    }
    // Validate the imported settings structure
    if (!settings.general || !settings.security || !settings.notifications || !settings.email) {
        throw new ErrorHandler_1.AppError('Invalid settings format', 400);
    }
...
exports.validateSettings = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const settingsData = req.body;
    const errors = [];

...
exports.exportReport = exports.getSystemHealth = exports.getActivityReport = exports.getTasksReport = exports.getFinancialReport = exports.getProjectsReport = exports.getUsersReport = exports.getReportStats = void 0;
...
const getReportStats = async (req, res) => {
    try {
        ...
        const [totalUsers, activeUsers, newUsers, totalProjects, activeProjects, completedProjects, totalTasks, completedTasks, pendingTasks, projectsWithBudget] = await Promise.all([
            prisma_1.default.user.count(),
            prisma_1.default.user.count({ where: { isActive: true } }),
            prisma_1.default.user.count({ where: { createdAt: { gte: cutoffDate } } }),
            ...
            // Fixed: Use correct enum values from schema
            prisma_1.default.task.count({ where: { status: 'DONE' } }),
            // Fixed: Use correct enum values from schema
            prisma_1.default.task.count({
        ...
        const completedProjectsData = await prisma_1.default.project.findMany({
            where: {
                status: 'COMPLETED',
                startDate: { not: null },
                endDate: { not: null }
...
const getUsersReport = async (req, res) => {
    try {
        const { range = '30d', limit = '50', page = '1' } = req.query;
        const days = getDaysFromRange(range);
        const cutoffDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
...
const getProjectsReport = async (req, res) => {
    try {
        const { range = '30d', limit = '50', page = '1', status, priority } = req.query;
        const days = getDaysFromRange(range);
        const cutoffDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
        const pageNum = parseInt(page);
        const limitNum = parseInt(limit);
        ...
        // Get total count for pagination
        const totalProjects = await prisma_1.default.project.count({ where: query });
...
const getProjectPriority = (tasks) => {
    if (tasks.length === 0)
        return 'MEDIUM';
...
const getProjectManager = (members) => {
    const adminMember = members.find(member => member.user.role === 'ADMIN' || member.user.role === 'SUPER_ADMIN');
    if (adminMember) {
        return {
...
const getFinancialReport = async (req, res) => {
    try {
        ...
        const financialData = Object.values(monthlyData).map((data) => {
        ...
        // Calculate totals
        const totals = financialData.reduce((acc, month) => ({
...
const getTasksReport = async (req, res) => {
    try {
        ...
        if (priority)
            query.priority = priority;
        // Get total count for pagination
        const totalTasks = await prisma_1.default.task.count({ where: query });
...
exports.getTasksReport = getTasksReport;
// Get activity report
...
const getSystemHealth = async (req, res) => {
    try {
        ...
        const [totalUsers, activeUsers, totalProjects, activeProjects, totalTasks, pendingTasks] = await Promise.all([
            prisma_1.default.user.count(),
            prisma_1.default.user.count({ where: { isActive: true } }),
            prisma_1.default.project.count(),
        ...
        const databaseStatus = await prisma_1.default.$queryRaw `SELECT 1 as connection_test`
            .then(() => 'healthy')
            .catch(() => 'unhealthy');
        const healthReport = {
...
async function getUsersExportData(filters) {
    const query = {};
    if (filters.role)
...
async function getProjectsExportData(filters) {
    const query = {};
    if (filters.status)
...
async function getTasksExportData(filters) {
    const query = {};
    if (filters.status)
        query.status = filters.status;
    if (filters.priority)
...
async function getFinancialExportData(filters) {
    const query = { budget: { gt: 0 } };
    if (filters.dateRange) {
        const days = getDaysFromRange(filters.dateRange);
        const cutoffDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
        query.createdAt = { gte: cutoffDate };
